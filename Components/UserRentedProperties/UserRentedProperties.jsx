...
function UserRentedProperties() {
  ...
  useEffect(() => {
    const fetchRented = async () => {
      ...
      const rentedRef = collection(doc(db, 'users', currentUser.uid), 'rentedProperties');
      setLoading(true)
  ...
  return (
    <div className="p-6 w-full mx-auto bg-white shadow-md rounded-md">
      <div className="text-center py-6 bg-gradient-to-r from-blue-500 to-purple-600 text-white rounded-md shadow-md">
        <h1 className="text-4xl font-extrabold">Your Rented Properties</h1>
        <p className="text-lg mt-2">Here are the properties you have rented!</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="mt-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {rented.length === 0 ? (
            <p className="text-center text-gray-500 italic col-span-full">No rented properties found yet.</p>
          ) : (
